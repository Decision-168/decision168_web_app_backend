...
router.get(
  "/user/get-all-counts/:email_id/:id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      const [rows5] = await pool.execute("CALL view_left_subtask_count(?)", [
        id,
      ]);
...
router.get(
  "/user/get-recent-notifications/:id",
  authMiddleware,
  async (req, res) => {
    const id = req.params.id;
    try {
...
router.get(
  "/user/get-alert-notifications/:id",
  authMiddleware,
  async (req, res) => {
    const id = req.params.id;
    try {
      ...
      //New task assigned
      const [rows1] = await pool.execute("CALL task_notification_clear(?)", [
        id,
      ]);
      ...
      const [rows5] = await pool.execute(
        "CALL check_task_review_sent_clear(?)",
        [id]
      );
      const SentToReviewTasks = rows5[0];

      //Review denied task
      const [rows6] = await pool.execute(
        "CALL check_task_review_deny_clear(?)",
        [id]
      );
      ...
      const [rows18] = await pool.execute("CALL PendingGoalList_clear(?)", [
        id,
      ]);
      ...
      const [aprojects] = await pool.execute("CALL AcceptedProjectList(?)", [
        id,
      ]);
...
//My Alert Page API's
router.get(
  "/user/get-my-alert-notifications/:id",
  authMiddleware,
  async (req, res) => {
    const id = req.params.id;
    try {
      ...
      //Sent to review task
      const [rows5] = await pool.execute("CALL check_task_review_sent(?)", [
        id,
      ]);
      const SentToReviewTasks = rows5[0];

      //Review denied task
      const [rows6] = await pool.execute("CALL check_task_review_deny(?)", [
        id,
      ]);
      const ReviewDeniedTasks = rows6[0];
      ...
      //Subtask Review Arrive
      const [rows12] = await pool.execute(
        "CALL check_subtask_arrive_review(?)",
        [id]
      );
      const ReviewArriveSubtasks = rows12[0];
