...
router.get("/goal/goal-detail-accepted/:user_id/:gid", async (req, res) => {
  ...
  const gid = req.params.gid;
  try {
    const [rows, fields] = await pool.execute("CALL GoalDetailAccepted(?,?)", [
      user_id,
      gid,
    ]);
    res.status(200).json(rows[0]);
...
router.get("/goal/file-it-strategy-tasks/:sid", async (req, res) => {
  ...
  try {
    const [rows, fields] = await pool.execute("CALL file_itStrategy_tasks(?)", [
      sid,
    ]);
    res.status(200).json(rows[0]);
  } catch (error) {
...
router.get("/goal/view-all-history-goal/:gid", async (req, res) => {
  const gid = req.params.gid;
  try {
    const [rows, fields] = await pool.execute("CALL view_all_history_goal(?)", [
      gid,
    ]);
    res.status(200).json(rows[0]);
  } catch (error) {
...
router.patch(
  "/goal/goal-open-work-new-assignee/:reg_id/:new_reg_id/:old_reg_id/:gmid_id/:portfolio_id",
  async (req, res) => {
    const reg_id = req.params.reg_id;
    const new_reg_id = req.params.new_reg_id;
    const old_reg_id = req.params.old_reg_id;
    const gmid_id = req.params.gmid_id;
    const portfolio_id = req.params.portfolio_id;
    try {
...
//delete_iGMember
router.patch(
  "/goal/remove-goal-invited-member/:user_id/:igm_id/:gid/:sent_to",
  async (req, res) => {
    try {
      const user_id = req.params.user_id;
      const igm_id = req.params.igm_id;
      const gid = req.params.gid;
      const sent_to = req.params.sent_to;
