...
router.get(
  "/file-cabinet/data/:portfolio_id/:user_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      const getSubtaskFiles = async (stid, portfolio_dept_id) => {
        ...
        if (subtasks_files[0]) {
          ...
          subtask_files_parent = (
            await Promise.all(subtask_files_promises)
          ).flat();
        }
      ...
      const getDepartmentGoal = async (portfolio_dept_id) => {
        let goal_parent = [];
        const [goals_1] = await pool.execute(
          "CALL file_itGoalsDeptWise(?,?,?)",
      ...
      const getDepartmentTask = async (portfolio_dept_id) => {
        let task_parent = [];
        const [tasks_1] = await pool.execute(
          "CALL file_itTasksDeptWise(?,?,?)",
          [user_id, portfolio_dept_id, portfolio_id]
        );
...
router.get(
  "/file-cabinet/get-student-detail/:user_id",
  authMiddleware,
  async (req, res) => {
    const { user_id } = req.params;
    try {
      const [user_row] = await pool.execute("CALL getStudentById(?)", [
        user_id,
      ]);
...
router.get(
  "/file-cabinet/files/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { portfolio_id } = req.params;
    try {
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      singlesubtask_files[0].forEach(
        async (ssbtf) => (ssbtf.file_type = "subtask")
      );
...
router.get(
  "/file-cabinet/project-detail/:pid",
  authMiddleware,
  async (req, res) => {
...
router.get(
  "/file-cabinet/project-files-detail/:pfile_id",
  authMiddleware,
  async (req, res) => {
...
router.get(
  "/file-cabinet/subtask-detail/:stid",
  authMiddleware,
  async (req, res) => {
...
router.get(
  "/file-cabinet/department-detail/:deptId",
  authMiddleware,
  async (req, res) => {
    const { deptId } = req.params;
    try {
      const [department_detail] = await pool.execute(
...
router.get(
  "/file-cabinet/goal-kpi-detail/:gid/:portfolio_dept_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { gid, portfolio_dept_id, portfolio_id } = req.params;
    try {
      const [goal_kpi_detail] = await pool.execute(
        "CALL file_itStrategiesCountGoalWise(?,?,?)",
        [gid, portfolio_dept_id, portfolio_id]
      );
...
router.get(
  "/file-cabinet/kpi-project-detail/:reg_id/:sid/:portfolio_dept_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { reg_id, sid, portfolio_dept_id, portfolio_id } = req.params;
    try {
      const [project_detail] = await pool.execute(
        "CALL file_itProjectsStrategyWise(?,?,?,?)",
        [reg_id, sid, portfolio_dept_id, portfolio_id]
      );
      const [aproject_detail] = await pool.execute(
        "CALL file_itAcceptedProjectsStrategyWise(?,?,?,?)",
        [reg_id, sid, portfolio_dept_id, portfolio_id]
      );
...
router.get(
  "/file-cabinet/task-subtask-detail/:reg_id/:tid/:portfolio_dept_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      const [single_subtask_detail] = await pool.execute(
        "CALL file_itSingleSubtaskCountTaskWise(?,?,?,?)",
        [reg_id, tid, portfolio_dept_id, portfolio_id]
      );
      const task_subtask_detail = [
        ...subtask_detail[0],
        ...single_subtask_detail[0],
      ];
...
router.get(
  "/file-cabinet/portfolio-detail/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { portfolio_id } = req.params;
    try {
      const [portfolio_detail] = await pool.execute(
