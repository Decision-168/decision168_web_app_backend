...
router.get("/project/get-project-list/:user_id/:portfolio_id", authMiddleware, async (req, res) => {
  ...
  try {
    const [regularList] = await pool.execute("CALL ProjectListByPortfolioRegular(?,?)", [portfolio_id, user_id]);
    const [acceptedList] = await pool.execute("CALL AcceptedProjectListByPortfolioRegular(?,?)", [portfolio_id, user_id]);
    const [pendingList] = await pool.execute("CALL PendingProjectListByPortfolioRegular(?,?)", [portfolio_id, user_id]);
    const [readMoreList] = await pool.execute("CALL ReadMoreProjectListByPortfolioRegular(?,?)", [portfolio_id, user_id]);
    ...
    if (regularListData) {
      regularList_promises = regularListData.map(async (row) => {
        ...
        let bell_icon = "no";
        const [notify1] = await pool.execute("CALL check_notify_project_management(?,?)", [row.pid, user_id]);
        const checkNotify1 = notify1[0][0];
        const [notify2] = await pool.execute("CALL check_notify_project_suggested(?)", [row.pid]);
...
router.get("/project/get-dashboard-project-list/:user_id/:portfolio_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [acceptedList] = await pool.execute("CALL AcceptedProjectListRegular(?)", [user_id]);
    const [pendingList] = await pool.execute("CALL PendingProjectListRegular(?)", [user_id]);
    const [readMoreList] = await pool.execute("CALL ReadMoreProjectListRegular(?)", [user_id]);
...
router.get("/project-request/:pid/:pm_id/:flag", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (flag == 1) {
      const [result] = await pool.execute("CALL check_ProPMToClear(?)", [pm_id]);
      if (result[0].length > 0) {
        const status = result[0][0]?.status;

        const [rows] = await pool.execute("CALL getStudentById(?)", [result[0][0]?.pmember]);
        ...
        if (status == "send" || status == "read_more") {
          ...
          await pool.execute("CALL UpdateProjectMembers(?, ?)", [dynamicFieldsValues, id]);
          ...
          await pool.execute("CALL UpdateProjectPortfolioMember(?, ?)", [dynamicFieldsValues2, id2]);
...
router.get("/project/get-project-by-id/:pid", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [project_wise_all_tasks] = await pool.execute("CALL progress_total(?)", [project_id]);
    const [project_wise_done_tasks] = await pool.execute("CALL progress_done(?)", [project_id]);

    const [project_wise_all_subtasks] = await pool.execute("CALL sub_progress_total(?)", [project_id]);
    ...
    const [get_portfolio] = await pool.execute("CALL getPortfolio2(?)", [project_detail.portfolio_id]);
    const get_portfolio_createdby_id = get_portfolio[0][0]?.portfolio_createdby;

    const [getDeptName] = await pool.execute("CALL get_PDepartment(?)", [project_detail.dept_id]);
    const get_dept_name = getDeptName[0][0]?.department;

    const [getCreatedByName] = await pool.execute("CALL getStudentById(?)", [project_detail.pcreated_by]);
    const get_created_by_name = getCreatedByName[0][0].first_name + " " + getCreatedByName[0][0].last_name;

    let get_pmanager_name = "";
    if (project_detail.pmanager != 0) {
      const [getManagerName] = await pool.execute("CALL getStudentById(?)", [project_detail.pmanager]);
      get_pmanager_name = getManagerName[0][0].first_name + " " + getManagerName[0][0].last_name;
    }

    const [ProjectTeamMember] = await pool.execute("CALL ProjectTeamMember(?)", [project_id]);
    const [InvitedProjectMember] = await pool.execute("CALL InvitedProjectMember(?)", [project_id]);
    const [SuggestedProjectMember] = await pool.execute("CALL SuggestedProjectMember(?)", [project_id]);

    const [SuggestedInviteProjectMember] = await pool.execute("CALL SuggestedInviteProjectMember(?)", [project_id]);
  ...
  try {
    ...
    if (taskDetail) {
      projectTaskAssignee_promises = taskDetail.map(async (item) => {
        const [all_tasks] = await pool.execute("CALL progress_total3(?,?)", [pid, item.tassignee]);
        const [done_tasks] = await pool.execute("CALL progress_done3(?,?)", [pid, item.tassignee]);
        const [tAssigneeDetail] = await pool.execute("CALL getStudentById(?)", [item.tassignee]);
    ...
    if (subtaskDetail) {
      projectSubtaskAssignee_promises = subtaskDetail.map(async (item) => {
        const [all_subtasks] = await pool.execute("CALL sub_progress_total3(?,?)", [pid, item.stassignee]);
        const [done_subtasks] = await pool.execute("CALL sub_progress_done3(?,?)", [pid, item.stassignee]);
        const [stAssigneeDetail] = await pool.execute("CALL getStudentById(?)", [item.stassignee]);
    ...
    const [projectTaskAssignee_parent, projectSubtaskAssignee_parent] = await Promise.all([Promise.all(projectTaskAssignee_promises), Promise.all(projectSubtaskAssignee_promises)]);
...
router.get("/project/project-files/:pid", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [projectFile_parent, taskFile_parent, subtaskFile_parent] = await Promise.all([Promise.all(projectFile_promises), Promise.all(taskFile_promises), Promise.all(subtaskFile_promises)]);
...
router.get("/project/get-project-comments/:pid/:user_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [projectComment_parent] = await Promise.all([Promise.all(projectComment_promises)]);
...
router.get("/project/project-notifications/:pid/:user_id", async (req, res) => {
  const { pid, user_id } = req.params;
  try {
    ...
    const [pMember_row] = await pool.execute("CALL ProjectTeamMember(?)", [pid]);
    const pMemberData = pMember_row[0];
    pMemberData?.map(async (row) => {
      const pmId = row.pm_id;
      const [pNotify_row] = await pool.execute("CALL get_project_accepted_notification(?,?)", [user_id, pmId]);
    ...
    const [pIMember_row] = await pool.execute("CALL InvitedProjectMember(?)", [pid]);
        ...
        imemail.map(async (im) => {
          ...
          if (check_if_registered[0].length > 0) {
            ...
            if (check_email[0].length == 0) {
              ...
              const get_pdes = getProject.pdes;
              const short_pdes = get_pdes.substring(0, 100);
              const acceptRequest = `${config.verificationLink}project-invite-reject-request/${getProject.pid}/${im_id}/1`;
              const rejectRequest = `${config.verificationLink}project-invite-reject-request/${getProject.pid}/${im_id}/2`;
              const position = "team member";
              const mailOptions2 = {
                from: process.env.SMTP_USER,
                to: im,
      ...
      await Promise.all(
        team_member.map(async (t) => {
          ...
          if (check_Project_members[0].length == 0) {
            ...
            const short_pdes = get_pdes.substring(0, 100);
            const acceptRequest = `${config.verificationLink}project-request/${pid}/${pm_id}/1`;
            const rejectRequest = `${config.verificationLink}project-request/${pid}/${pm_id}/2`;
            const position = "team member";
            const mailOptions2 = {
              from: process.env.SMTP_USER,
              to: user.email_address,
        ...
        imemail.map(async (im) => {
          ...
          if (check_if_registered[0].length > 0) {
            ...
            if (check_email[0].length == 0) {
              ...
              const get_pdes = pdetail.pdes;
              const short_pdes = get_pdes.substring(0, 100);
              const acceptRequest = `${config.verificationLink}project-invite-reject-request/${pid}/${im_id}/1`;
              const rejectRequest = `${config.verificationLink}project-invite-reject-request/${pid}/${im_id}/2`;
              const position = "team member";
              const mailOptions2 = {
                from: process.env.SMTP_USER,
                to: im,
...
router.patch("/project/project-open-work-new-assignee", authMiddleware, async (req, res) => {
  ...
  try {
    const [check_pm] = await pool.execute("CALL check_ProPMToClear(?)", [pm_id]);
...
router.patch("/project/add-suggested-project-member/:user_id/:pid/:suggest_id", authMiddleware, async (req, res) => {
  try {
    ...
    if (check_project_members[0].length == 0) {
      ...
      const get_pdes = pdetail.pdes;
      const short_pdes = get_pdes.substring(0, 100);
      const acceptRequest = `${config.verificationLink}project-request/${pdetail.pid}/${pm_id}/1`;
      const rejectRequest = `${config.verificationLink}project-request/${pdetail.pid}/${pm_id}/2`;
      const position = "team member";
      const mailOptions2 = {
        from: process.env.SMTP_USER,
        to: user.email_address,
  ...
  try {
    ...
    if (check_if_registered[0].length > 0) {
      ...
      if (check_email[0].length == 0) {
        ...
        const get_pdes = pdetail.pdes;
        const short_pdes = get_pdes.substring(0, 100);
        const acceptRequest = `${config.verificationLink}project-invite-reject-request/${pdetail.pid}/${im_id}/1`;
        const rejectRequest = `${config.verificationLink}project-invite-reject-request/${pdetail.pid}/${im_id}/2`;
        const position = "team member";
        const mailOptions2 = {
          from: process.env.SMTP_USER,
          to: im,
  ...
  try {
    ...
    const promises = rows[0].map(async (item) => {
      ...
      if (getName && getName[0] && getName[0][0]) {
        let check_pmem = "";
        const [check_pmRes] = await pool.execute("CALL check_pm(?,?,?)", [pid, portfolio_id, getName[0][0].reg_id]);
...
router.get("/project/get-all-accepted-goal-team-member-project-list/:portfolio_id/:pid/:gid/:user_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const promises = rows[0].map(async (item) => {
      const [getName] = await pool.execute("CALL getStudentById(?)", [item.gmember]);
      let data;
      if (getName && getName[0] && getName[0][0]) {
        let check_pmem = "";
        const [check_pmRes] = await pool.execute("CALL check_pm(?,?,?)", [pid, portfolio_id, getName[0][0].reg_id]);
    ...
    if (imemail && imemail.length > 0) {
      await Promise.all(
        imemail.map(async (im) => {
          ...
          if (check_if_registered[0].length > 0) {
            const rid = check_if_registered[0][0]?.reg_id;
            const [check_Project_members] = await pool.execute("CALL check_ProjectMToClear(?,?)", [rid, pid]);
            if (check_Project_members[0].length == 0) {
              if (pcreated_by != rid) {
                const data7 = {
                  pid: pid,
                ...
                const [check_portfolio] = await pool.execute("CALL check_PortfolioMember(?,?)", [im, getProject.portfolio_id]);
...
router.get("/project/get-project-create-dd/:portfolio_id/:gid/:user_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [Portfolio] = await pool.execute("CALL getPortfolioName(?)", [portfolio_id]);
    ...
    if (gid != 0) {
      ...
      const [PortfolioDepartment] = await pool.execute("CALL get_PDepartment(?)", [gdept]);
      ...
      const [AssignList] = await pool.execute("CALL GoalTeamMemberAccepted(?)", [gid]);
      ...
      const [PortfolioDepartment] = await pool.execute("CALL get_PortfolioDepartment(?)", [portfolio_id]);
