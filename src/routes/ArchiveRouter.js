...
router.get("/archive/:portfolio_id/:user_id", async (req, res) => {
  ...
  try {
    ...
    const getSubtaskFiles = async (stid, portfolio_dept_id) => {
      ...
      const [subtasks_files] = await pool.execute("CALL file_itSubtaskFilesTaskWise(?,?,?,?)",
    ...
    const getTaskSubtasks = async (tid, portfolio_dept_id) => {
      let subtask_parent = [];
      const [subtasks] = await pool.execute("CALL file_itSubtaskCountTaskWise(?,?,?,?)",
    ...
    const getProjectTasks = async (pid, portfolio_dept_id) => {
      let task_parent = [];
      const [tasks] = await pool.execute("CALL file_itTasksCountProjectWise(?,?,?,?)",
    ...
    const getKpiProject = async (sid, portfolio_dept_id) => {
      let project_parent = [];
      const [projects_1] = await pool.execute("CALL file_itProjectsCountStrategyWise(?,?,?,?)",
        [user_id, sid, portfolio_dept_id, portfolio_id]
      );
      const [projects_2] = await pool.execute("CALL file_itAcceptedProjectsCountStrategyWise(?,?,?,?)",
        [user_id, sid, portfolio_dept_id, portfolio_id]
      );
    ...
    const getGoalKpi = async (gid,portfolio_dept_id) => {
      let kpi_parent = [];
      const [kpis] = await pool.execute("CALL file_itStrategiesCountGoalWise(?,?,?)",
    ...
    const getDepartmentGoal = async (portfolio_dept_id) => {
      ...
      const [goals_2] = await pool.execute("CALL file_itAcceptedGoalsDeptWise(?,?,?)",
        [user_id, portfolio_dept_id, portfolio_id]
      );
    ...
    const getDepartmentKpi = async (portfolio_dept_id) => {
      let kpi_parent = [];
      const [kpis_1] = await pool.execute("CALL file_itStrategiesDeptWise(?,?,?)",
        [user_id, portfolio_dept_id, portfolio_id]
      );
    
      const [kpis_2] = await pool.execute("CALL file_itAcceptedStrategiesDeptWise(?,?,?)",
        [user_id, portfolio_dept_id, portfolio_id]
      );
    ...
    const getDepartmentProject = async (portfolio_dept_id) => {
      let project_parent = [];
      const [projects_1] = await pool.execute("CALL file_itProjectsDeptWise(?,?,?)",
        [user_id, portfolio_dept_id, portfolio_id]
      );
      const [projects_2] = await pool.execute("CALL file_itAcceptedProjectsDeptWise(?,?,?)",
        [user_id, portfolio_dept_id, portfolio_id]
      );
    ...
    const getDepartmentTask = async (portfolio_dept_id) => {
      ...
      const [tasks_2] = await pool.execute("CALL file_itSingleTasksDeptWise(?,?,?)",
        [user_id, portfolio_dept_id, portfolio_id]
      );
    ...
    const getDepartmentSubtask = async (portfolio_dept_id) => {
      let subtask_parent = [];
      const [subtasks_1] = await pool.execute("CALL file_itSubtasksDeptWise(?,?,?)",
        [user_id, portfolio_dept_id, portfolio_id]
      );
      const [subtasks_2] = await pool.execute("CALL file_itSingleSubtasksDeptWise(?,?,?)",
        [user_id, portfolio_dept_id, portfolio_id]
      );
...
// Get All Portfolio departments
router.get("/archive/files/:portfolio_id", async (req, res) => {
...
// File it Goal
router.patch("/archive/file-it/goal/:goal_id/:user_id", async (req, res) => {
    ...
    try {
      ...
      if (goal_row[0][0]) {
        ...
        if (total_all == total_done) {
          ...
          const [owner_row] = await pool.execute(
            "CALL getStudentById(?)",
            [user_id]
          );
...
// File it KPI
router.patch("/archive/file-it/kpi/:strategy_id/:user_id", async (req, res) => {
      ...
      if (kpi_row[0][0]) {
        ...
        if (total_all == total_done) {
          ...
          return res
            ...
            .json({
              error:
                "Please Complete All Tasks and Subtasks to File it the KPI!",
            });
        }
      } else {
...
// File it Project
router.patch("/archive/file-it/project/:project_id/:user_id", async (req, res) => {
...
// File it Task
router.patch("/archive/file-it/task/:task_id/:user_id", async (req, res) => {
      ...
      if (task_row[0][0]) {
        ...
        if (all_subtask == done_subtask) {
          ...
          return res
            ...
            .json({
              error:
                "Please Complete All Subtasks to File it the Task.",
            });
        }
      } else {
...
// File it Subtask
router.patch("/archive/file-it/subtask/:subtask_id/:user_id", async (req, res) => {
...
// Reopen Goal
router.patch("/archive/reopen/goal/:goal_id/:portfolio_id/:user_id", async (req, res) => {
...
// Reopen KPI
router.patch("/archive/reopen/kpi/:strategy_id/:portfolio_id/:user_id", async (req, res) => {
  ...
  try {
    ...
    if(limitation == 'in_limit') { 
      const [kpi_row] = await pool.execute("CALL check_strategy_file_it(?,?)", [
        user_id, strategy_id
      ]);
      if (kpi_row[0][0]) {  
        const [goal_row] = await pool.execute( "CALL checkStrategyGoalfile_it(?)", [kpi_row[0][0].gid]);
        if(goal_row[0][0]){
          res.status(400).json({ error: "Goal is Filed! To Reopen KPI please Reopen Goal." });
        }else{
              const kpiFieldsValues = `s_file_it = '', s_file_it_date = ''`;
              const sid = `sid = '${strategy_id}'`;
              await pool.execute("CALL UpdateStrategies(?, ?)", [ kpiFieldsValues, sid ]);
...
// Reopen Project
router.patch("/archive/reopen/project/:project_id/:portfolio_id/:user_id", async (req, res) => {
  ...
  try {
    ...
    if(limitation == 'in_limit') { 
      ...
      if (project_row[0][0]) {  
        ...
        if(kpi_row[0][0]){
          res.status(400).json({ error: "KPI is Filed! To Reopen Project please Reopen KPI." });
        }else{
              const projectFieldsValues = `project_file_it = '', project_file_it_date = ''`;
              const pid = `pid = '${project_id}'`;
              await pool.execute("CALL UpdateProject(?, ?)", [projectFieldsValues, pid]);
...
// Reopen Task
router.patch("/archive/reopen/task/:task_id/:portfolio_id/:user_id", async (req, res) => {
  ...
  try {
    ...
    if(limitation == 'in_limit') { 
      ...
      if (task_row[0][0]) {  
        ...
        if(project_row[0][0]){
          res.status(400).json({ error: "Project is Filed! To Reopen Task please Reopen Project." });
        }else{
          const taskFieldsValues = `task_file_it = '', task_file_it_date = ''`;
          const tid = `tid = '${task_id}'`;
          await pool.execute("CALL UpdateTask(?, ?)", [
...
// Reopen Subtask
router.patch("/archive/reopen/subtask/:subtask_id/:user_id", async (req, res) => {
  const { subtask_id, user_id } = req.params;
  try {
    const [owner_row] = await pool.execute("CALL getStudentById(?)", [user_id]);
    const student = owner_row[0][0];
    const formattedDate = dateConversion();
      ...
      if (subtask_row[0][0]) {  
        ...
        if(task_row[0][0]){
          res.status(400).json({ error: "Task is Filed! To Reopen Subtask please Reopen Task." });
        }else{
          const subtaskFieldsValues = `subtask_file_it = '', subtask_file_it_date = ''`;
          const stid = `stid = '${subtask_id}'`;
          await pool.execute("CALL UpdateSubtask(?, ?)", [
...
router.get("/archive/recent-files/:user_id/:portfolio_id", async (req, res) => {
