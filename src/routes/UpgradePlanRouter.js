...
//getAllPack
router.get(
  "/upgrade-plan/get-all-pack/:user_id",
  authMiddleware,
  async (req, res) => {
...
//get_active_coupons
router.get(
  "/upgrade-plan/get-active-coupons",
  authMiddleware,
  async (req, res) => {
    try {
...
router.post(
  "/upgrade-plan/free-trial-account-access",
  authMiddleware,
  async (req, res) => {
...
//insert_ContactSales
router.post(
  "/upgrade-plan/insert-contact-sales",
  authMiddleware,
  async (req, res) => {
    const { pass_total_users, user_id } = req.body;
    try {
      const [check_contactedRes] = await pool.execute(
        "CALL check_contacted(?)",
        [user_id]
      );
...
router.post(
  "/upgrade-plan/create-checkout-session",
  authMiddleware,
  async (req, res) => {
...
router.post(
  "/upgrade-plan/insert-checkout-payment-data",
  authMiddleware,
  async (req, res) => {
...
router.post(
  "/upgrade-plan/update-all-users-package-details",
  async (req, res) => {
    try {
      const currentDate = moment().format("YYYY-MM-DD HH:mm:ss");
