...
router.get(
  "/trash/all-data/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { user_id, portfolio_id } = req.params;
    try {
      const [trash_goals] = await pool.execute("CALL TrashGoals(?,?)", [
        user_id,
        portfolio_id,
      ]);
      const [trash_kpis] = await pool.execute("CALL TrashStrategies(?,?)", [
        user_id,
        portfolio_id,
      ]);
      const [trash_projects] = await pool.execute("CALL TrashProjects(?,?)", [
        portfolio_id,
        user_id,
      ]);
      const [trash_tasks] = await pool.execute("CALL TrashTasks(?,?)", [
        user_id,
        portfolio_id,
      ]);
      const [trash_single_tasks] = await pool.execute(
        "CALL TrashSingleTasks(?,?)",
        [user_id, portfolio_id]
      );
      const [trash_subtasks] = await pool.execute("CALL TrashSubtasks(?,?)", [
        portfolio_id,
        user_id,
      ]);
      const [trash_single_subtasks] = await pool.execute(
        "CALL TrashSingleSubtasks(?,?)",
        [portfolio_id, user_id]
      );
      const [trash_ProjectFiles] = await pool.execute(
        "CALL TrashProjectFiles(?)",
        [portfolio_id]
      );
      const [trash_TaskFiles] = await pool.execute("CALL TrashTaskFiles(?)", [
        portfolio_id,
      ]);
      const [trash_SubtaskFiles] = await pool.execute(
      ...
      if (trash_projects_data) {
        ...
        trash_projects_parent = (
          await Promise.all(trash_projects_promises)
        ).flat();
      }
      ...
      if (trash_single_tasks_data) {
        const trash_single_tasks_promises = trash_single_tasks_data.map(
          async (row) => {
            const [portfolio_del] = await pool.execute(
              "CALL getPortfolioById(?)",
              [row.portfolio_id]
        ...
        trash_single_tasks_parent = (
          await Promise.all(trash_single_tasks_promises)
        ).flat();
      }
      ...
      if (trash_subtasks_data) {
        ...
        trash_subtasks_parent = (
          await Promise.all(trash_subtasks_promises)
        ).flat();
      }
      ...
      if (trash_single_subtasks_data) {
        const trash_single_subtasks_promises = trash_single_subtasks_data.map(
          async (row) => {
            const [portfolio_del] = await pool.execute(
              "CALL getPortfolioById(?)",
              [row.portfolio_id]
            ...
            const formattedsingle_subtaskDate = jsDate
              .toISOString()
              .split("T")[0];
            return {
              id: `subtask-${row.stid}`,
        ...
        trash_single_subtasks_parent = (
          await Promise.all(trash_single_subtasks_promises)
        ).flat();
      }
      ...
      if (trash_projectFiles_data) {
        const trash_projectFiles_promises = trash_projectFiles_data.map(
          async (row) => {
            const [project_detail] = await pool.execute(
              "CALL file_itgetProjectById2(?)",
              [row.pid]
            );
        ...
        trash_projectFiles_parent = await Promise.all(
          trash_projectFiles_promises
        );
      }
      ...
      if (trash_memberFiles_data && trash_memberFiles_data.length > 0) {
        const trash_memberFiles_promises = trash_memberFiles_data.map(
          async (row) => {
            const [pmember_detail] = await pool.execute(
              "CALL file_itgetMemberProject(?)",
              [row.pid]
            );
        ...
        const processedData = (await Promise.all(trash_memberFiles_promises))
          .flat()
      ...
      if (trash_taskFiles_data) {
        const trash_taskFiles_promises = trash_taskFiles_data.map(
          async (row) => {
            const [task_detail] = await pool.execute(
              "CALL file_itgetProjectById2(?)",
              [row.pid]
            );
      ...
      if (trash_memberTaskFiles_data && trash_memberTaskFiles_data.length > 0) {
        const trash_memberTaskFiles_promises = trash_memberTaskFiles_data.map(
          async (row) => {
            const [pmember_detail] = await pool.execute(
              "CALL file_itgetMemberProject(?)",
              [row.pid]
        ...
        const processedData = (
          await Promise.all(trash_memberTaskFiles_promises)
        )
          .flat()
          .filter((item) => Object.keys(item).length > 0);
      ...
      if (trash_subtaskFiles_data) {
        const trash_subtaskFiles_promises = trash_subtaskFiles_data.map(
          async (row) => {
        ...
        trash_subtaskFiles_parent = await Promise.all(
          trash_subtaskFiles_promises
        );
      }
      ...
      if (
        trash_memberSubtaskFiles_data &&
        trash_memberSubtaskFiles_data.length > 0
      ) {
        const trash_memberSubtaskFiles_promises =
          trash_memberSubtaskFiles_data.map(async (row) => {
            const [pmember_detail] = await pool.execute(
              "CALL file_itgetMemberProject(?)",
              [row.pid]
            );
              ...
              return check_pmember.map(async (row1) => {
                if (row1.status == "accepted" && row1.pmember == user_id) {
                  return {
                    ...
                    file_name: row.stfile,
                    all_portfolio: portfolioName,
                    all_trash: row.stfile.substring(
                      row.stfile.indexOf("_") + 1
                    ),
                    all_title: row.stname,
                    all_type: "Subtask File",
                    all_date: formattedsubtaskDate,
        ...
        const processedData = (
          await Promise.all(trash_memberSubtaskFiles_promises)
        )
          .flat()
          .filter((item) => Object.keys(item).length > 0);
...
router.get(
  "/trash/goal-data/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { user_id, portfolio_id } = req.params;
    try {
...
router.get(
  "/trash/kpi-data/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { user_id, portfolio_id } = req.params;
    try {
...
router.get(
  "/trash/project-data/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { user_id, portfolio_id } = req.params;
    try {
...
router.get(
  "/trash/task-data/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { user_id, portfolio_id } = req.params;
    try {
      ...
      if (trash_tasks_data) {
        const trash_tasks_promises = trash_tasks_data.map(async (row) => {
          ...
          const [assignee_del] = await pool.execute("CALL getStudentById(?)", [
            row.tassignee,
          ]);
      ...
      if (trash_single_tasks_data) {
        const trash_single_tasks_promises = trash_single_tasks_data.map(
          async (row) => {
            ...
            const [assignee_del] = await pool.execute(
              "CALL getStudentById(?)",
              [row.tassignee]
            );
            const assigneeName = `${assignee_del[0][0].first_name} ${assignee_del[0][0].last_name}`;
...
router.get(
  "/trash/files-data/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { user_id, portfolio_id } = req.params;
    ...
    try {
      ...
      if (trash_projectFiles_data) {
        trash_projectFiles_promises = trash_projectFiles_data.map(
          async (row) => {
            const [project_detail] = await pool.execute(
              "CALL file_itgetProjectById2(?)",
              [row.pid]
      ...
      if (trash_taskFiles_data) {
        const trash_memberTaskFiles_data = trash_TaskFiles[0];
        trash_memberTaskFiles_promises = trash_memberTaskFiles_data.map(
          async (row) => {
            const [pmember_detail] = await pool.execute(
              "CALL file_itgetMemberProject(?)",
              [row.pid]
      ...
      if (trash_subtaskFiles_data) {
        trash_subtaskFiles_promises = trash_subtaskFiles_data.map(
          async (row) => {
            const [subtask_detail] = await pool.execute(
              "CALL file_itgetProjectById2(?)",
              [row.pid]
      ...
      if (trash_subtaskFiles_data) {
        const trash_memberSubtaskFiles_data = trash_SubtaskFiles[0];
        trash_memberSubtaskFiles_promises = trash_memberSubtaskFiles_data.map(
          async (row) => {
            const [pmember_detail] = await pool.execute(
              "CALL file_itgetMemberProject(?)",
              [row.pid]
            ...
            if (check_pmember) {
              check_pmember.forEach(async (row1) => {
                if (row1.status == "accepted" && row1.pmember == user_id) {
                  const subtaskDate = row.stask_trash_date;
                  const jsDate = new Date(subtaskDate);
                  const formattedsubtaskDate = jsDate
                    .toISOString()
                    .split("T")[0];
                  ...
                  return {
                    ...
                    file_name: row.stfile,
                    file_portfolio: portfolioName,
                    file_file: row.stfile.substring(
                      row.stfile.indexOf("_") + 1
                    ),
                    file_title: row.stname,
                    file_type: "Subtask File",
...
// Delete Portfolio
router.patch(
  "/trash/delete/portfolio/:portf_id/:user_id",
  authMiddleware,
  async (req, res) => {
...
// Delete Goal
router.patch(
  "/trash/delete/goal/:goal_id/:user_id",
  authMiddleware,
  async (req, res) => {
...
// Delete KPI
router.patch(
  "/trash/delete/kpi/:strategy_id/:user_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      if (kpi_row[0][0]) {
        ...
        const historyFieldsValues = `"${strategy_id}", "${
          ...
          student.first_name
        } ${student.last_name}", "KPI Moved to Trash By KPI Owner ${
          student.first_name
        } ${student.last_name}"`;
...
// Delete Project
router.patch(
  "/trash/delete/project/:project_id/:user_id",
  authMiddleware,
  async (req, res) => {
    const { project_id, user_id } = req.params;
    try {
      const [project_row] = await pool.execute(
        "CALL file_itProjectDetail(?,?)",
        [project_id, user_id]
      );
...
// Delete Task
router.patch(
  "/trash/delete/task/:task_id/:user_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      if (task_row[0][0]) {
        ...
        const historyFieldsValues = `"${task_id}", "${
          ...
          task_row[0][0].tcode
        } Task Moved to Trash By ${student.first_name} ${student.last_name}"`;
...
// Delete Subtask
router.patch(
  "/trash/delete/subtask/:subtask_id/:user_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      if (subtask_row[0][0]) {
        ...
        } Subtask Moved to Trash By ${student.first_name} ${
          student.last_name
        }"`;
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (task_row[0][0]) {
        ...
        const taskTrashFieldsNames =
          "pid, tid, tfile, task_trash, task_trash_date";
        ...
        const new_tfile_array = tfile_array.filter(
          (obj) => obj !== task_file_name
        );
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (subtask_row[0][0]) {
        ...
        const historyFieldsNames =
          "subtask_id, pid, sid, gid, h_date, h_resource_id, h_resource, h_description";
        const historyFieldsValues = `"${subtask_id}", "${
          subtask_row[0][0].stproject_assign
        }", "${subtask_row[0][0].sid}", "${
          subtask_row[0][0].gid
          ...
          student.first_name
        } ${student.last_name}", "${subtask_file} Moved to Trash By ${
          student.first_name
        } ${student.last_name}"`;
        ...
        const subtaskTrashFieldsNames =
          "pid, stid, tid, stfile, stask_trash, stask_trash_date";
        ...
        const new_stfile_array = stfile_array.filter(
          (obj) => obj !== subtask_file_name
        );
        ...
        return res
          .status(200)
          .json({ message: "Subtask File Moved to Trash Successfully." });
      } else {
      ...
      if (trash_row[0][0]) {
        ...
        if (task_row[0][0]) {
          ...
          const historyFieldsValues = `"${task_id}", "${
            ...
            student.first_name
          } ${student.last_name}", "${task_file} Restored By ${
            student.first_name
          } ${student.last_name}"`;
          ...
          const tid = `tid = '${task_id}'`;
          await pool.execute("CALL UpdateTask(?,?)", [taskFieldsValues, tid]);
          await pool.execute("CALL DeleteTaskTrash(?)", [
            `trash_id = '${trash_id}'`,
          ]);
      ...
      if (trash_row[0][0]) {
        ...
        if (subtask_row[0][0]) {
          ...
          const historyFieldsValues = `"${subtask_id}", "${
            ...
            student.first_name
          } ${student.last_name}", "${subtask_file} Restored By ${
            student.first_name
          } ${student.last_name}"`;
          ...
          if (
            subtask_row[0][0].stfile &&
            subtask_row[0][0].stfile.trim() !== ""
          ) {
          ...
          await pool.execute("CALL DeleteSubtaskTrash(?)", [
            `strash_id = '${trash_id}'`,
          ]);
            ...
            if (kpi_wise_projects[0]) {
              ...
              project_array.forEach(async (row) => {
                ...
                if (tasks[0]) {
                  ...
                  await pool.execute("CALL DeleteTask(?)", [
                    `tproject_assign = '${pid}'`,
                  ]);
                }
                ...
                if (subtasks[0]) {
                  ...
                  await pool.execute("CALL DeleteSubtask(?)", [
                    `stproject_assign = '${pid}'`,
                  ]);
                }

                await pool.execute("CALL DeleteProjectFiles(?)", [
                  `pid = '${pid}'`,
                ]);
                await pool.execute("CALL DeleteProjectInvitedMembers(?)", [
                  `pid = '${pid}'`,
                ]);
                await pool.execute("CALL DeleteProjectManagement(?)", [
                  `pid = '${pid}'`,
                ]);
                await pool.execute("CALL DeleteProjectManagementFields(?)", [
                  `pid = '${pid}'`,
                ]);
                await pool.execute("CALL DeleteProjectMembers(?)", [
                  `pid = '${pid}'`,
                ]);
                await pool.execute("CALL DeleteProjectSuggestedMembers(?)", [
                  `pid = '${pid}'`,
                ]);
                await pool.execute("CALL DeleteProjectHistory(?)", [
                  `pid = '${pid}'`,
                ]);
                await pool.execute("CALL DeleteProjectRequestMember(?)", [
                  `pid = '${pid}'`,
                ]);
...
// Delete Forever KPI
router.patch(
  "/trash/delete-forever/kpi/:sid/:user_id",
  authMiddleware,
  async (req, res) => {
...
router.patch(
  "/trash/delete-forever/subtask/:stid",
  authMiddleware,
  async (req, res) => {
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      const [trash_row] = await pool.execute(
        "CALL check_tfile_task_trash(?,?)",
        [trash_id, task_id]
      );
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      const [trash_row] = await pool.execute(
        "CALL check_stfile_subtask_trash(?,?)",
