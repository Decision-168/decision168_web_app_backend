...
router.get("/portfolio/all-portfolios/:email_address/:user_id", authMiddleware, async (req, res) => {
  ...
  try {
    const [portfolioRowList] = await pool.execute("CALL get_SideBar_ALLPortfolio(?)", [email_address]);
    const [portfolioRowGrid] = await pool.execute("CALL get_SideBar_Portfolio(?)", [email_address]);
    ...
    if (portfolioDataGrid) {
      portfolioGrid_promises = portfolioDataGrid?.map(async (row) => {
        const companyName = row.portfolio_user === "company" ? row.portfolio_name : `${row.portfolio_name} ${row.portfolio_lname}`;
        const [creator] = await pool.execute("CALL getStudentById(?)", [row.portfolio_createdby]);
        const user = creator[0][0];

        const [projectRow] = await pool.execute("CALL count_portfolio_project(?)", [row.portfolio_id]);
        const projectCount = projectRow[0][0].count_rows;

        const [taskRow] = await pool.execute("CALL count_Portfolio_task(?)", [row.portfolio_id]);
    ...
    const [portfolioList_parent, portfolioGrid_parent] = await Promise.all([Promise.all(portfolioList_promises), Promise.all(portfolioGrid_promises)]);
...
router.get("/portfolio/get-all-portfolios/:email_address", authMiddleware, async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute("CALL get_SideBar_Portfolio(?)", [email_address]);
...
router.get("/portfolio/get-project-and-task-count/:portfolio_id", authMiddleware, async (req, res) => {
  const { portfolio_id } = req.params;

  try {
    const [[projectCount], [taskCount]] = await Promise.all([pool.execute("CALL count_portfolio_project(?)", [portfolio_id]), pool.execute("CALL count_Portfolio_task(?)", [portfolio_id])]);
...
router.get("/portfolio/get-all-accepted-portfolio-team-members/:portfolio_id", authMiddleware, async (req, res) => {
  const { portfolio_id } = req.params;
  try {
    const [rows] = await pool.execute("CALL getAll_Accepted_PortTM(?)", [portfolio_id]);
...
router.patch("/portfolio/change-portfolio-member-status/:pim_id/:portfolio_id", authMiddleware, async (req, res) => {
  const pim_id = req.params.pim_id;
  const portfolio_id = req.params.portfolio_id;
  const status = req.body.status;
  let statusChanged = false;
  try {
    const [check_ppm] = await pool.execute("CALL check_PPMToClear(?)", [pim_id]);
    ...
    if (check) {
      if (status === "active") {
        ...
        await pool.execute("CALL UpdateProjectPortfolioMember(?, ?)", [updateFieldsValues1, upid]);
        statusChanged = true;
        res.status(200).json({ message: "status changed successfully", statusChanged });
      }
      if (status === "inactive") {
        const [getpm] = await pool.execute("CALL selectLogin(?)", [check.sent_to]);
        const pm = getpm[0][0];
        if (pm) {
          const reg_id = pm.reg_id;

          const [getGoals] = await pool.execute("CALL TMOpenGoals(?,?)", [reg_id, portfolio_id]);
          let goal_count = 0;
          if (getGoals[0]) {
            goal_count = getGoals[0].length;
          }

          const [getGoalTM] = await pool.execute("CALL getGoalOpenTM(?,?)", [reg_id, portfolio_id]);
          let goal_tm_count = 0;
          if (getGoalTM[0]) {
            goal_tm_count = getGoalTM[0].length;
          }

          const [getStrategies] = await pool.execute("CALL TMOpenStrategies(?,?)", [reg_id, portfolio_id]);
          let strategies_count = 0;
          if (getStrategies[0]) {
            strategies_count = getStrategies[0].length;
          }

          const [getProjects] = await pool.execute("CALL TMOpenProjects(?,?)", [reg_id, portfolio_id]);
          let pro_count = 0;
          ...
          const [getTasks] = await pool.execute("CALL TMOpenTasks(?,?)", [reg_id, portfolio_id]);
          let task_count = 0;
          if (getTasks[0]) {
            task_count = getTasks[0].length;
          }

          const [getSubtasks] = await pool.execute("CALL TMOpenSubtasks(?,?)", [reg_id, portfolio_id]);
          let subtask_count = 0;
          if (getSubtasks[0]) {
            subtask_count = getSubtasks[0].length;
          }

          const [getProjectTM] = await pool.execute("CALL getProjectOpenTM(?,?)", [reg_id, portfolio_id]);
          let pro_tm_count = 0;
          if (getProjectTM[0]) {
            pro_tm_count = getProjectTM[0].length;
          }

          if (goal_count === 0 && strategies_count === 0 && pro_count === 0 && task_count === 0 && subtask_count === 0 && pro_tm_count === 0 && goal_tm_count === 0) {
...
router.patch("/portfolio/open-work-new-assignee/:reg_id/:new_reg_id/:old_reg_id/:pim_id/:portfolio_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (check) {
      ...
      const [getGoalsRes] = await pool.execute("CALL TMOpenGoals(?,?)", [old_reg_id, portfolio_id]);
      const getGoals = getGoalsRes[0];
      if (getGoals) {
        for (const gg of getGoals) {
          if (gg.gcreated_by == old_reg_id) {
            const [checkIfGoalTM] = await pool.execute("CALL TMOpenGoals(?,?)", [new_reg_id, gg.gid]);
            if (checkIfGoalTM[0]) {
            ...
            await pool.execute("CALL UpdateGoals(?, ?)", [updateFieldsValues, upid]);
          ...
          if (gg.gmanager == old_reg_id) {
            ...
            await pool.execute(callProcedureSQL, [paramNamesString1, paramValuesString1]);
          }

          // Check if team member in any goal
          const [checkTM] = await pool.execute("CALL checkOpenGoalTM(?,?)", [old_reg_id, gg.gid]);
      ...
      const [getGoalTMRes] = await pool.execute("CALL getGoalOpenTM(?,?)", [old_reg_id, portfolio_id]);
      const getGoalTM = getGoalTMRes[0];
      if (getGoalTM) {
        for (const ggtm of getGoalTM) {
          const [check_if_already_goaltmRes] = await pool.execute("CALL check_if_already_goaltm(?,?,?)", [new_reg_id, ggtm.gid, portfolio_id]);

          const check_if_already_goaltm = check_if_already_goaltmRes[0];
          if (check_if_already_goaltm.length == 0) {
            const [check_if_goalownerRes] = await pool.execute("CALL check_if_goalowner(?,?)", [new_reg_id, ggtm.gid]);
      ...
      const [getStrategiesRes] = await pool.execute("CALL TMOpenStrategies(?,?)", [old_reg_id, portfolio_id]);
      ...
      const [getProjectsRes] = await pool.execute("CALL TMOpenProjects(?,?)", [old_reg_id, portfolio_id]);
      ...
      const [getProjectTMRes] = await pool.execute("CALL getProjectOpenTM(?,?)", [old_reg_id, portfolio_id]);
      const getProjectTM = getProjectTMRes[0];
      if (getProjectTM) {
        for (const gtm of getProjectTM) {
          const [check_if_already_tmRes] = await pool.execute("CALL check_if_already_tm(?,?,?)", [new_reg_id, gtm.pid, portfolio_id]);

          const check_if_already_tm = check_if_already_tmRes[0];
          if (check_if_already_tm.length == 0) {
            const [check_if_pownerRes] = await pool.execute("CALL check_if_powner(?,?)", [new_reg_id, gtm.pid]);
      ...
      const [getTasksRes] = await pool.execute("CALL TMOpenTasks(?,?)", [old_reg_id, portfolio_id]);
      ...
      const [getSubtasksRes] = await pool.execute("CALL TMOpenSubtasks(?,?)", [old_reg_id, portfolio_id]);
...
router.get("/portfolio/get-portfolio-details/:portfolio_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const { portfolio_createdby } = rows[0][0];

    const [creator] = await pool.execute("CALL getStudentById(?)", [portfolio_createdby]);
    const portflioCreatedByName = `${creator[0][0]?.first_name} ${creator[0][0]?.last_name}`;

    const [department] = await pool.execute("CALL get_PortfolioDepartment(?)", [portfolio_id]);
    const departmentNames = department[0].map((item) => item.department);

    const portfolioDetails = {
      ...rows[0][0],
      portflioCreatedByName,
...
router.get("/portfolio/get-portfolio-departments/:portfolio_id", authMiddleware, async (req, res) => {
  const { portfolio_id } = req.params;
  try {
    const [rows] = await pool.execute("CALL get_PortfolioDepartment(?)", [portfolio_id]);
    return res.status(200).json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: "Internal server error." });
  }
...
router.post("/portfolio/insert-project-portfolio-department", authMiddleware, async (req, res) => {
  try {
    ...
    if (departments && departments.length > 0 && cus_departments && cus_departments.length == 0) {
      all_dept.push(...departments);
    } else if (departments && departments.length == 0 && cus_departments && cus_departments.length > 0) {
    ...
    if (all_dept && all_dept.length > 0) {
      await Promise.all(
        all_dept.map(async (dept) => {
          ...
          await pool.execute(callProcedureSQLPortDept, [paramNamesStringPortDept, paramValuesStringPortDept]);
        })
      );
    }
...
router.patch("/portfolio/update-portfolio-department/:id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await pool.execute("CALL UpdateProjectPortfolioDepartment(?, ?)", [updateData, portfolio_dept_id]);
    ...
    if (imemail && imemail.length > 0) {
      await Promise.all(
        imemail.map(async (sent_to) => {
          ...
          const [checkEmail] = await pool.execute("CALL checkPortfolioMemberEmail(?,?)", [sent_to, portfolio_id]);
          if (checkEmail[0]?.length > 0) {
            ...
            const [getPID] = await pool.execute("CALL checkPortfolioMemberEmail(?,?)", [sent_to, portfolio_id]);
            const PIM_id = getPID[0][0]?.pim_id;
            const [check_powner] = await pool.execute("CALL getStudentById(?)", [sent_from]);
            const powner = check_powner[0][0];
            ...
            const acceptRequest = `${config.verificationLink}portfolio-invite-request/${portfolio_id}/${PIM_id}/1`;
            const rejectRequest = `${config.verificationLink}portfolio-invite-request/${portfolio_id}/${PIM_id}/2`;

            const mailOptions = {
              from: process.env.SMTP_USER,
              to: sent_to,
              subject: "Portfolio Team Member Request | Decision 168",
              html: generatePortfolioRequestEmailTemplate(pownerFName, PortfolioName, AboutPortfolio, acceptRequest, rejectRequest),
            };
...
router.get("/portfolio-invite-request/:portfolio_id/:pim_id/:flag", authMiddleware, async (req, res) => {
  const { portfolio_id, pim_id, flag } = req.params;
  try {
    if (flag == 1) {
      const [getportd] = await pool.execute("CALL getPortfolioMember(?,?)", [pim_id, portfolio_id]);
      if (getportd[0].length > 0) {
        const email_address = getportd[0][0]?.sent_to;
        const status = getportd[0][0]?.status;
        const [check_if_registered] = await pool.execute("CALL selectLogin(?)", [email_address]);
        if (status == "pending") {
...
router.get("/portfolio/get-all-accepted-active-portfolio-team-members/:portfolio_id", authMiddleware, async (req, res) => {
  const { portfolio_id } = req.params;
  try {
    const [rows] = await pool.execute("CALL getAll_Accepted_PortTM(?)", [portfolio_id]);
    const promises = rows[0].map(async (item) => {
