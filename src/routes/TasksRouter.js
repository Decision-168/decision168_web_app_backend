...
router.get("/task/project-tasks-list/:project_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Use Promise.all to concurrently fetch subtasks for each task
    const promisesTask = project_tasks_listNew[0].map(async (item) => {
      const { tid } = item;
      try {
        const [subtasks] = await pool.execute("CALL  Check_Task_Subtasks2(?)", [tid]);
        const subTasks = subtasks[0];
    ...
    res.status(200).json({
      ...
      tlist: tasklistResults,
    });

    // Check_Task_Subtasks2
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
});
...
router.get("/get-projects-list/:portfolio_id/:user_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const simplifiedProjects = projects.map(({ pid, pname, dept_id, gid }) => ({
      pid,
      pname,
      dept_id,
      gid,
