...
router.get(
  "/task/all-tasks-subtasks-grid-view/:reg_id",
  authMiddleware,
  async (req, res) => {
    const { reg_id } = req.params;
    try {
      // Retrieve assigned task list portfolio
      const [assignedTasklist] = await pool.execute(
        "CALL AssignedTasklist(?)",
        [reg_id]
      );
      ...
      const promisesTasks = assignedTasklist[0].map(async (taskItem) => {
        ...
        try {
          ...
          const projectName =
            projectDetails[0].length > 0 ? projectDetails[0][0].pname : null;
      ...
      const promisesSubtasks = assignedSubtasklist[0].map(
        async (subtaskItem) => {
      ...
      const combinedArray = [
        ...
        ...subtasks.map((subtask) => ({
          ...
          parentTaskName:
            tasks.find((task) => task.tid === subtask.tid)?.tname || null,
          type: "subtask",
        })),
      ];
...
router.get(
  "/task/all-tasks-subtasks-list-view/:reg_id",
  authMiddleware,
  async (req, res) => {
    const { reg_id } = req.params;
    //(Pagination Code)
    ...
    try {
      ...
      const promisesMainTask = assignedSubtasklist_Task[0].map(async (item) => {
        const { tid } = item;
        try {
          const [taskDetails] = await pool.execute("CALL getTasksDetail(?)", [
            tid,
          ]);
          const [subtasks] = await pool.execute(
            "CALL  Check_Task_Subtasks2(?)",
      ...
      const removeDuplicates = (result) => {
        return result.filter((item, index, array) => {
          ...
          return (
            array.findIndex((element) => element.tid === item.tid) === index
          );
        });
      };
...
router.get(
  "/task/portfolio-tasks-subtasks-list-view/:portfolio_id/:reg_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      // Use Promise.all to concurrently fetch subtasks for each task
      const promisesMainTask = assignedSubtasklist_TaskPortfolio[0].map(
        async (item) => {
          const { tid } = item;
          try {
            const [taskDetails] = await pool.execute("CALL getTasksDetail(?)", [
...
router.get(
  "/task/portfolio-tasks-subtasks-grid-view/:portfolio_id/:reg_id",
  authMiddleware,
  async (req, res) => {
    const { portfolio_id, reg_id } = req.params;
    ...
    try {
      // Retrieve assigned task list portfolio
      const [assignedTasklistPortfolio] = await pool.execute(
        "CALL AssignedTasklistPortfolio(?,?)",
        [portfolio_id, reg_id]
      );
      ...
      // Use Promise.all to concurrently fetch subtasks for each task
      const promisesTasks = assignedTasklistPortfolio[0].map(
        async (taskItem) => {
          const { tid, tproject_assign } = taskItem;
          try {
            const [projectDetails] = await pool.execute(
      ...
      // Use Promise.all to concurrently fetch subtasks for each subtask
      const promisesSubtasks = assignedSubtasklistPortfolio[0].map(
        async (subtaskItem) => {
          const { stproject_assign } = subtaskItem;
          try {
            const [projectDetails] = await pool.execute(
...
router.get(
  "/task/portfolio-tasks/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      const [portfolioTasks] = await pool.execute(
        "CALL portfolio_tasksNew(?)",
        [portfolio_id]
      );

      const portfolioTasksPromises = portfolioTasks[0].map(
        async (portfolioTaskItem) => {
          const { tid, tassignee } = portfolioTaskItem;

          try {
            const [taskAssigneeDetails] = await pool.execute(
              "CALL getStudentById(?)",
              [tassignee]
            );
            ...
            const subtasksPromises = subtasks[0].map(async (subtaskItem) => {
              ...
              const [subtaskAssigneeDetails] = await pool.execute(
                "CALL getStudentById(?)",
                [stassignee]
              );
...
router.get(
  "/task/portfolio-tasks-list/:portfolio_id/:reg_id",
  authMiddleware,
  async (req, res) => {
    const { portfolio_id, reg_id } = req.params;
...
router.get("/task/task-detail/:task_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (taskDetails[0][0]) {
      const { tproject_assign, portfolio_id, tcreated_by, tassignee } =
        taskDetails[0][0];

      const [getUserDetailsByTcreatedBy] = await pool.execute(
        "CALL getStudentById(?)",
        [tcreated_by]
      );

      const [getUserDetailsbyTassignee] = await pool.execute(
        "CALL getStudentById(?)",
        [tassignee]
      );

      const [projectDetails] = await pool.execute("CALL getProjectById(?)", [
        tproject_assign,
      ]);
...
// Subtask Details
router.get(
  "/subtask/subtask-detail/:subtask_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      if (subtaskDetails[0][0]) {
        const { stproject_assign, portfolio_id, stcreated_by, stassignee } =
          subtaskDetails[0][0];
...
router.post("/task/insert-task/:user_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    let taskInsertedId;
    if (student) {
      const [team_member2_row] = await pool.execute("CALL getStudentById(?)", [
        team_member2,
      ]);
      const links_string = links
        .map((linkObj) => Object.values(linkObj).join(","))
        .join(",");
      const link_comments_string = link_comments
        .map((linkObj) => Object.values(linkObj).join(","))
        .join(",");
      const tfile_string = tfile
        .map((linkObj) => Object.values(linkObj).join(","))
        .join(",");
      ...
      if (
        ...
        if (pro_owner != team_member2) {
          if (index === -1) {
            ...
            const acceptProjectRequest = `${config.verificationLink}project-request/${project_id}/${inserted_pm_id}/1`;
            const rejectProjectRequest = `${config.verificationLink}project-request/${project_id}/${inserted_pm_id}/2`;
        ...
        if (pro_owner != team_member2) {
          if (index === -1) {
            const [check] = await pool.execute("CALL check_suggested(?,?)", [
              project_id,
              team_member2,
            ]);
      ...
      const [inserted_task] = await pool.execute("CALL LastInsertTask(?)", [
        user_id,
      ]);
      const inserted_task_id = inserted_task[0][0].tid;
      taskInsertedId = inserted_task_id;
      const historyFieldsNames =
        "pid, gid, sid, h_date, h_resource_id, h_resource, h_description, task_id";
...
// Insert Subtask
router.post(
  "/subtask/insert-subtask/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
            ...
            if (
              ...
              if (pro_owner != item.team_member2) {
                if (index === -1) {
                  const [check_if_suggested] = await pool.execute(
                    "CALL check_suggested(?,?)",
                    [tproject_assign, item.team_member2]
                  );
                  if (check_if_suggested[0][0]) {
                    const suggestTMFieldsValues = `status = 'approved', approve_date = '${formattedDate}'`;
                  ...
                  //     const acceptProjectRequest = `${config.verificationLink}project-request/${tproject_assign}/${inserted_pm_id}/1`;
                  //     const rejectProjectRequest = `${config.verificationLink}project-request/${tproject_assign}/${inserted_pm_id}/2`;
                  //     const mailOptions = {
                  //       from: process.env.SMTP_USER,
                  //       to: RequestEmailID,
                  //       subject: "Project Request | Decision 168",
                  ...
                  const acceptRequest = `${config.verificationLink}project-request/${pdetail.pid}/${inserted_pm_id}/1`;
                  const rejectRequest = `${config.verificationLink}project-request/${pdetail.pid}/${inserted_pm_id}/2`;
                  const position = "team member";
                  const mailOptions = {
                    from: process.env.SMTP_USER,
                    to: RequestEmailID,
...
//Edit Subtask
router.post(
  "/subtask/edit-subtask/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      if (student) {
        ...
        const slinks_string = slinks
          .map((linkObj) => Object.values(linkObj).join(","))
          .join(",");
        const slink_comments_string = slink_comments
          .map((linkObj) => Object.values(linkObj).join(","))
          .join(",");
        const stfile_string = stfile
          .map((linkObj) => Object.values(linkObj).join(","))
          .join(",");
        ...
        if (check_subtask) {
          ...
          const results = await Promise.all(
            taskArray.map(async (item) => {
              ...
              if (
                ...
                if (pro_owner != item.team_member2) {
                  if (index === -1) {
                    ...
                    if (check_portfolio_name) {
                      if (
                        check_portfolio_name[0][0].portfolio_user ==
                        "individual"
                      ) {
                        get_portfolio_name = `${check_portfolio_name[0][0].portfolio_name} ${check_portfolio_name[0][0].portfolio_lname}`;
                      } else {
              ...
              const acceptProjectRequest = `${config.verificationLink}project-request/${stproject_assign}/${inserted_pm_id}/1`;
              const rejectProjectRequest = `${config.verificationLink}project-request/${stproject_assign}/${inserted_pm_id}/2`;
              const mailOptions = {
                from: process.env.SMTP_USER,
                to: RequestEmailID,
                subject: "Project Request | Decision 168",
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [taskDetail] = await pool.execute(
        "CALL check_task_assignProOwner(?)",
        [tid]
      );
      ...
      if (status_but == "to_do" || status_but == "in_progress") {
        (status_but == "to_do" && (status = "To Do")) ||
          (status_but == "in_progress" && (status = "In Progress"));
        ...
        const [check_st] = await pool.execute(
          "CALL subtask_progress_total(?)",
          [tid]
        );
        const [check_stdone] = await pool.execute(
...
router.patch(
  "/task/checkbox-change-status/:user_id",
  authMiddleware,
  async (req, res) => {
    const { user_id } = req.params;
    const { tid, tassignee } = req.body;
    try {
      const [taskDetail] = await pool.execute("CALL check_task_assign(?,?)", [
        tid,
        tassignee,
      ]);
      const task_detail = taskDetail[0][0];
      ...
      if (
        task_detail.tstatus == "to_do" ||
        task_detail.tstatus == "in_progress" ||
        task_detail.tstatus == "in_review"
      ) {
        const [check_st] = await pool.execute(
          "CALL subtask_progress_total(?)",
          [tid]
...
router.patch(
  "/subtask/checkbox-change-status/:user_id",
  authMiddleware,
  async (req, res) => {
    const { user_id } = req.params;
...
router.get(
  "/task/team-member-tasks-list/:project_id/:task_assignee",
  authMiddleware,
  async (req, res) => {
    const { project_id, task_assignee } = req.params;
    try {
      const [ProjectDetailCheck] = await pool.execute(
        "CALL ProjectDetailCheck(?)",
        [project_id]
      );
      const [team_member_tasks_listNew] = await pool.execute(
        "CALL team_member_tasks_listNew(?,?)",
        [project_id, task_assignee]
      );
      const [team_member_subtasks_listNew] = await pool.execute(
        "CALL team_member_subtasks_listNew(?,?)",
        [project_id, task_assignee]
      );
      res.status(200).json({
        pid: project_id,
...
router.get(
  "/task/portfolios-edit-task/:user_id",
  authMiddleware,
  async (req, res) => {
    const { user_id } = req.params;
    ...
    try {
      ...
      const [ProjectListbyPortCookie] = await pool.execute(
        "CALL ProjectListbyPortCookie(?,?)",
        [portfolio_id, user_id]
      );
      const [AcceptedProjectListbyPortCookie] = await pool.execute(
        "CALL AcceptedProjectListbyPortCookie(?,?)",
        [portfolio_id, user_id]
      );
      res.status(200).json({
...
//get projects
router.get(
  "/get-projects-list/:portfolio_id/:user_id",
  authMiddleware,
  async (req, res) => {
    const { user_id, portfolio_id } = req.params;
    ...
    try {
      ...
      const projects = [
        ...ProjectListbyPortCookie[0],
        ...AcceptedProjectListbyPortCookie[0],
      ];
...
//  Project Task list
router.get(
  "/task/project-tasks-list/:project_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      const [project_tasks_listNew] = await pool.execute(
        "CALL project_tasks_listNew(?)",
        [project_id]
      );
      res.status(200).json({
        pid: project_id,
...
//Trash Task files
router.get(
  "/task/trash-tasks-files/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { portfolio_id } = req.params;
    try {
...
router.patch(
  "/task/table-editable/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      if (txt) {
        ...
        if (div_class == "task_editable") {
          const [task_detail] = await pool.execute("CALL getTaskById(?)", [
            div_id,
          ]);
              ...
              if (
                ...
                if (pro_owner != txt) {
                  if (index === -1) {
                    ...
                    const inserted_pm_id = inserted_pm[0][0].pm_id;
                    const [txt_row] = await pool.execute(
                      "CALL getStudentById(?)",
                      [txt]
                    );
                    ...
                    const acceptProjectRequest = `${config.verificationLink}project-request/${tdetail.tproject_assign}/${inserted_pm_id}/1`;
                    const rejectProjectRequest = `${config.verificationLink}project-request/${tdetail.tproject_assign}/${inserted_pm_id}/2`;
                    const mailOptions = {
                      from: process.env.SMTP_USER,
                      to: RequestEmailID,
                      subject: "Project Request | Decision 168",
              ...
              if (
                ...
                if (pro_owner != txt) {
                  if (index === -1) {
                    ...
                    const acceptProjectRequest = `${config.verificationLink}project-request/${tdetail.stproject_assign}/${inserted_pm_id}/1`;
                    const rejectProjectRequest = `${config.verificationLink}project-request/${tdetail.stproject_assign}/${inserted_pm_id}/2`;
                    const mailOptions = {
                      from: process.env.SMTP_USER,
                      to: RequestEmailID,
                      subject: "Project Request | Decision 168",
...
router.post("/task/duplicate-task", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (check_Task) {
      const [student_detail] = await pool.execute("CALL getStudentById(?)", [
        user_id,
      ]);
      ...
      if (copy_detail == "everything" || copy_detail == "custom") {
        const formattedDueDate = check_Task.tdue_date
          .toISOString()
          .slice(0, 19)
          .replace("T", " ");
        const task_data = {
        ...
        if (cust_tws == "2" && copy_detail == "custom") {
          ...
          if (Check_Task_Subtasks) {
            Check_Task_Subtasks.forEach(async (ts) => {
              ...
              const [inserted_subtask] = await pool.execute(
                "CALL LastInsertSubTask(?)",
                [user_id]
              );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [inserted_cm] = await pool.execute("CALL LastInsertedComment(?)", [
        user_id,
      ]);
...
//  Delete Comment
router.get(
  "/task/delete-comment/:user_id/:cid",
  authMiddleware,
  async (req, res) => {
...
//  Insert Subtask File
router.patch(
  "/subtask/insert-subtask-file/:user_id",
  authMiddleware,
  async (req, res) => {
    const { user_id } = req.params;
    ...
    try {
      ...
      const student = getMydetail[0][0];
      const [subtask_row] = await pool.execute("CALL getSubtasksDetail(?)", [
        stid,
      ]);
...
router.get("/task/today-tasks/:user_id", authMiddleware, async (req, res) => {
  ...
  try {
    const [TodayTasks] = await pool.execute("CALL TodayTasks(?,?)", [
      user_id,
      tdue_date,
    ]);
    const [TodaySubtasklist_Task] = await pool.execute(
      "CALL TodaySubtasklist_Task(?,?)",
      [user_id, tdue_date]
    );
    const [TodaySubtasks] = await pool.execute("CALL TodaySubtasks(?,?)", [
      user_id,
      tdue_date,
    ]);
    res.status(200).json({
...
router.get("/task/week-tasks/:user_id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [WeekTasks] = await pool.execute("CALL WeekTasks(?,?,?)", [
      user_id,
      formattedFirstDay,
      formattedLastDay,
    ]);
    const [WeekSubtaskslist_Task] = await pool.execute(
      "CALL WeekSubtaskslist_Task(?,?,?)",
      [user_id, formattedFirstDay, formattedLastDay]
    );
    const [WeekSubtasks] = await pool.execute("CALL WeekSubtasks(?,?,?)", [
      user_id,
      formattedFirstDay,
      formattedLastDay,
    ]);
...
//  Tasks List
router.get(
  "/task/tasks-list/:user_id/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { user_id, portfolio_id } = req.params;
    try {
      const [createdTaskList] = await pool.execute(
        "CALL CreatedTaskList(?,?)",
        [portfolio_id, user_id]
      );
      const [createdSubTaskList] = await pool.execute(
        "CALL CreatedSubTaskList(?)",
        [portfolio_id, user_id]
      );
      res.status(200).json({
...
router.get("/task/kpi-tasks", authMiddleware, async (req, res) => {
  const { sid } = req.body;
  try {
    const [Strategy_tasks] = await pool.execute("CALL Strategy_tasks(?)", [
      sid,
    ]);
    res.status(200).json({
...
//get Task Comments
router.get(
  "/task/get-task-comments/:tid/:user_id",
  authMiddleware,
  async (req, res) => {
...
//getSubtaskComments
router.get(
  "/subtask/get-subtask-comments/:subtask_id/:user_id",
  authMiddleware,
  async (req, res) => {
    const { subtask_id, user_id } = req.params;
    try {
      const [rows] = await pool.execute("CALL getSubtaskComments(?)", [
        subtask_id,
      ]);
      ...
      const [subtaskComment_parent] = await Promise.all([
        Promise.all(subtaskComment_promises),
      ]);
...
router.post("/subtask/duplicate-subtask", authMiddleware, async (req, res) => {
  ...
  try {
    const formattedDate = dateConversion();
    const [subtask_detail] = await pool.execute("CALL SubtaskDetail(?)", [
      stid,
    ]);
...
router.get("/subtask/user-subtasks", authMiddleware, async (req, res) => {
  const { reg_id } = req.body;
  try {
    const [get_all_subtask] = await pool.execute("CALL get_all_subtask(?)", [
      reg_id,
    ]);
    res.status(200).json({
...
router.get("/subtask/kpi-subtasks", authMiddleware, async (req, res) => {
  const { sid } = req.body;
  try {
    const [Strategy_subtasks] = await pool.execute(
      "CALL Strategy_subtasks(?)",
      [sid]
    );
    res.status(200).json({
...
router.get(
  "/subtask/portfolio-subtasks/:portfolio_id",
  authMiddleware,
  async (req, res) => {
    const { portfolio_id } = req.params;
    try {
